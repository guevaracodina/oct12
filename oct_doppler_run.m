...
for acquisition = 1:size(OCTmat,1) % Acquisitions loop
    try % //EGC
        load(OCTmat{acquisition});
        if( ~isfield(OCT.jobsdone,'doppler') || job.redo )
            % Reconstruction parameters to save in recons_info structure.
            ...
            for iFiles = 1:acqui_info.nfiles, % Save several .dopl3D files // EGC
                ...
                for i_frame = 1:acqui_info.nframes;
                    if i_frame>size(recons_info.A_line_position,2)
                        warning('oct_doppler_run:exceedingFrame',...
                            'Requested frame number exceeds number of frames, please verify')
                        keyboard
                    end
                    if i_frame==2
                        tic
                    ...
                    waitbarmessage={['Acq. ' num2str(acquisition) ' of '...
                        ...
                        [ETA_text  ', Frame ' num2str(i_frame+(current_file-1)*acqui_info.nframes) ' of '...
                    ...
                    if acqui_info.nfiles > 1 && i_frame==1 % //EGC
                        fprintf('Processing %d of %d files\n', current_file, acqui_info.nfiles);
                    end
                ...
                % Save several files //EGC
                if acqui_info.nfiles > 1
                    tmp_fnm = strcat(tmp_fnm, '-', num2str(i_frame*current_file));
                end
