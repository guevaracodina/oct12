...
reconstruct1.name = 'Reconstruct normal doppler';             % The display name
reconstruct1.tag  = 'oct_doppler_cfg'; % The name appearing in the harvested job structure. This name must be unique among all items in the val field of the superior node
reconstruct1.val  = {OCTmat dop1};    % The items that belong to this branch. All items must be filled before this branch can run or produce virtual outputs
reconstruct1.prog = @oct_doppler_run;  % A function handle that will be called with the harvested job to run the computation
reconstruct1.vout = @oct_doppler_vout; % A function handle that will be called with the harvested job to determine virtual outputs
reconstruct1.help = {'Start from preprocessed data and reconstructs structural acquisition.'};

%% Local Functions
% For output dependency
function vout = oct_doppler_vout(job)
